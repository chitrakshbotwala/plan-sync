...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<void> loginWithGoogle() async {
    ...
    try {
      ...
      if (currentUser != null) {
        await CrashReporter.setUserIdentifier(currentUser!.uid);
      }
  ...
  Future<void> logout() async {
    ...
    await _auth.signOut();
    if (!kIsWeb) {
      // Web signs in via signInWithPopup, so there is no GoogleSignIn session
      // to clear — and google_sign_in_web rejects our serverClientId config.
      await _ensureGoogleInitialized();
